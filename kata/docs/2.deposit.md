...
We added a new use case to improve our confidence to the implementation.
In this case we could use a `property`

```text
for all amount
such as account existing and amount <= 0
property depositUseCase.invoke mustBe "Invalid amount for deposit"
```

Let's keep for a next session 😜
